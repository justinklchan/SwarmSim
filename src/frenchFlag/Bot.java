...
//        return closest;
    ...
    {
        if(x+sOffsetX < 0 || y-sOffsetY < 0 || 
           y-sOffsetY >= img.length || x+sOffsetX >= img[0].length)
        {
            return WHITE;
        }
        else
        {
            ...
            try
            {
                u = img[img.length-y-1-sOffsetY][x+sOffsetX];
            }
            catch(Exception e)
            {
                System.out.println(clusterID+" "+(x+sOffsetX));
            }
            return u;
        }
    }
